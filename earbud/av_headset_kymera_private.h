...
enum app_kymera_internal_message_ids
{
    ...
    /*! Internal EQ update message */
    KYMERA_INTERNAL_A2DP_UPDATE_EQ,
    #endif
};
...
typedef struct
{
    ...
    /*! Should SCO be started with SCO Forwarding (if that is supported) */
    bool allow_scofwd;
...
*/
void appKymeraHandleInternalScoStart(Sink sco_sink, hfp_wbs_codec_mask codec,
...
/*! \brief Set operator use case ids.
    ...
    \param is_narrowband_sco Set TRUE if the SCO use case is narrowband, FALSE for wideband.
*/
...
#if defined(INCLUDE_EQ)
void appKymeraHandleInternalA2dpUpdateEq(void);
#endif
