...
static bool appPowerNeedsToPowerOff(void)
{
    ...
    DEBUG_LOGF("appPowerNeedsToPowerOff temperature(%d) inited(%d) battery(%d)", temperature_extreme, user_initiated, battery_too_low);
...
void appPowerOffTimerRestart(void)
{
    ...
    if (APP_POWER_EVENT_NONE == thePower->power_events_mask){
        ...
        MessageSendLater(appGetPowerTask(), APP_POWER_INTERNAL_AUTO_POWER_OFF, 0, appConfigIdleTimeoutMs());
    }
}
...
static void appPowerHandlePhyStateChanged(const PHY_STATE_CHANGED_IND_T *message)
{
    ...
    if (PHY_STATE_OUT_OF_EAR_AT_REST != message->new_state){
        appPowerOffTimerRestart();
    }
}


static void appPowerHandleIdleTimer(void)
{
    powerTaskData *thePower = appGetPower();

    DEBUG_LOGF("appPowerHandleIdleTimer (state:%d, phystate:%d)", appGetState(), appPhyStateGetState());

    if(appGetUi()->is_dut){
        DEBUG_LOGF("in dut(%d)", appGetUi()->is_dut);
        return;
    }
