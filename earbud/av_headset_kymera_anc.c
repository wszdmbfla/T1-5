...
void appKymeraAncTuningCreateChain(uint16 usb_rate)
{
    ...
    theKymera->usb_rx = (Operator)PanicFalse(VmalOperatorCreate(CAP_ID_USB_AUDIO_RX));
    PanicFalse(VmalOperatorMessage(theKymera->usb_rx,
                                   usb_config, SIZEOF_OPERATOR_MESSAGE(usb_config),
                                   NULL, 0));

    theKymera->usb_tx = (Operator)PanicFalse(VmalOperatorCreate(CAP_ID_USB_AUDIO_TX));
    ...
    if (mic_in1)
        PanicFalse(SourceSynchronise(fb_mon0, mic_in1));
