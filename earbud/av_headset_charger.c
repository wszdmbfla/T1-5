...
enum av_headset_charger_internal_messages
{
    ...
    CHARGER_INTERNAL_PROMPT_POWER_ON,
};
...
static void appChargerCheck(void)
{
    ...
    if (is_connected != theCharger->is_connected)
    {
        ...
        if (is_connected == CHARGER_DISCONNECTED)
        {
            ...
            if(appGetUi()->is_dut){
               DEBUG_LOG("reboot Go to DUT mode");
               appUiDUTReboot();
              // appUiChargerIsDUT();
            }
            else{
                if(appPowerOffRequest()){
        ...
        else
        {
            ...
            if (theCharger->is_connected == CHARGER_DISCONNECTED){
                ...
                MessageSend(appGetUiTask(), CHARGER_MESSAGE_ATTACHED, NULL);
             }
    ...
    DEBUG_LOGF("appBatteryGetVoltage:%d,con=%d,charger=%d,status=%d", appBatteryGetVoltage(), is_connected, is_charging, theCharger->status);
    ...
    if (is_connected == CHARGER_CONNECTED_NO_ERROR)
    {
        ...
        if (!is_charging && theCharger->is_charging)
		{
            ...
            /*tell ui charger complete then power off*/
            MessageCancelAll(appGetUiTask(), CHARGER_MESSAGE_CHARGER_COMPLETE);
            MessageSendLater(appGetUiTask(), CHARGER_MESSAGE_CHARGER_COMPLETE, 0, 50);
            
        }
        else if (is_charging)
        {
            if (appBatteryGetVoltage() > appConfigBatteryVoltageOk()){
                if(FALSE == appUiIsinFactoryReset())
...
static void appChargerHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case CHARGER_INTERNAL_PROMPT_POWER_ON:
