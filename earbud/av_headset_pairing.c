...
#define EARBUD_VALID_NAME_NUM 5
uint8 earbudValidName[EARBUD_VALID_NAME_NUM][25] = {
   "KPro01_L",
   "KPro01_R",
   "Joytech",
   "Joytech3020",
...
static void appPairingEnterIdle(pairingTaskData *thePairing)
{
    ...
    appPowerOffTimerEnable(APP_POWER_EVENT_PAIRING);

    /* Unlock pairing and permit a pairing operation */
    thePairing->pairing_lock = 0;

    if(appDeviceIsHandsetConnected() ||appPeerSyncIsPeerHandsetAvrcpConnected()){
        appUiIdleInactive();
    }
    else{
        appUiIdleActive();
...
static void appPairingExitIdle(pairingTaskData *thePairing)
{
    ...
    appPowerOffTimerDisable(APP_POWER_EVENT_PAIRING);
...
static void appPairingSetState(pairingTaskData *thePairing, pairingState state)
{
    ...
    if(state < PAIRING_STATE_HANDSET_DISCOVERABLE && state != PAIRING_STATE_IDLE ){
        appUiIdleInactive();
    }
...
static void appPairingHandsetComplete(pairingTaskData *thePairing, pairingStatus status, const bdaddr *bd_addr)
{
    ...
    {
        if(appPeerSyncIsComplete() && (appDeviceIsPeerAvrcpConnectedForAv()||appPeerSyncIsPeerHandsetAvrcpConnected())){
           DEBUG_LOG("Current is Slave pairing handset ,Don't paly prompts - pairing success");
        }
        else
...
{
    pairingTaskData *thePairing = appGetPairing();
    static int nb_pairing_attempt = 0; 
        ...
        case PAIRING_STATE_HANDSET_PENDING_AUTHENTICATION:
        {
            ...
            else
            {
               ...
               // appPairingHandsetComplete(thePairing, pairingHandsetAuthenticationFailed, &cfm->bd_addr);
                ...
                /* First time it fails we ignore it and try to page again to workaround B-286732   changed by bao */
                 if ( (cfm->status == auth_status_timeout || cfm->status == auth_status_fail) \
                 && PAIRING_RETRY_ALLOWED(nb_pairing_attempt) ) 
                 {
                    smTaskData *sm = appGetSm(); 
                    if(cfm->status == auth_status_fail) //added by bao 
                        nb_pairing_attempt = MAX_NUM_PAIRING_ATTEMPTS; 
                    else // Only want to pair up to MAX_NUM_PAIRING_ATTEMPTS 
                        nb_pairing_attempt ++; 
                   DEBUG_LOGF("appPairingHandleClSmAuthenticateCfm, authentication failure: trying to pair again (%d attempt(s) remaining)", (MAX_NUM_PAIRING_ATTEMPTS-nb_pairing_attempt)); 

                   // Reset to IDLE state to start again a clean pairing 
                   appPairingSetState(thePairing, PAIRING_STATE_IDLE); 
                   // Trigger a new pairing attempt 
                   appPairingHandsetPairAddress(&sm->task, &cfm->bd_addr); 
                 }
                 else { 
                 /* Send confirmation with error to main task */ 
                appPairingHandsetComplete(thePairing, pairingHandsetAuthenticationFailed, &cfm->bd_addr); 
                 
                /* We reached the max number of pairing attempts, indicate the failure and reset the counter */ 
                if (nb_pairing_attempt == MAX_NUM_PAIRING_ATTEMPTS) { 
                 nb_pairing_attempt = 0; 
                 } 
                } 
            }
...
#ifdef APP_CHECK_DEV_NAME_VALIDATION
bool appCheckDevNameValidation(void)
{
    uint8 i=0;
    bool status = FALSE;
    uint8 strBuf[25];
    ...
    //DEBUG_LOGF("name :%c%c%c%c%c\r\n", thePairing->localname[0],thePairing->localname[1],thePairing->localname[2],thePairing->localname[3],thePairing->localname[4]);
    //DEBUG_LOGF("name[0]:%c%c%c%c\r\n", earbudValidName[0][0],earbudValidName[0][1],earbudValidName[0][2],earbudValidName[0][3]);
    //DEBUG_LOGF("name[1]:%c%c%c%c\r\n", earbudValidName[1][0],earbudValidName[1][1],earbudValidName[1][2],earbudValidName[1][3]);
    //DEBUG_LOGF("name[2]:%c%c%c%c\r\n", earbudValidName[2][0],earbudValidName[2][1],earbudValidName[2][2],earbudValidName[2][3]);
    //DEBUG_LOGF("name[3]:%c%c%c%c\r\n", earbudValidName[3][0],earbudValidName[3][1],earbudValidName[3][2],earbudValidName[3][3]);
    //DEBUG_LOGF("name[4]:%c%c%c%c\r\n", earbudValidName[4][0],earbudValidName[4][1],earbudValidName[4][2],earbudValidName[4][3]);

    for(i=0;i<EARBUD_VALID_NAME_NUM;i++)
    {
    	DEBUG_LOGF("name:size:%d, %c%c%c%c\r\n",sizeof(earbudValidName[i]),earbudValidName[i][0],earbudValidName[i][1],earbudValidName[i][2],earbudValidName[i][3]);
    	
    	memcpy(strBuf, (const uint8 *)&earbudValidName[i][0], sizeof(earbudValidName[i]));
    	
      if(memcmp(thePairing->localname, strBuf, strlen((char *)strBuf)) == 0)
      {
         status = TRUE;
         break;
      }
...
static void appHandleGetLoalName(pairingTaskData *thePairing,CL_DM_LOCAL_NAME_COMPLETE_T *msg)
{
    uint8 length = 0;
    if(msg->size_local_name >= APP_LOCAL_DEV_NAME_LEN-1)
    	  length = APP_LOCAL_DEV_NAME_LEN;
    else
        length = msg->size_local_name;
    
    DEBUG_LOGF("appHandleClDmLocalNameComplete, name:%x,%x,%x,%x", msg->local_name[0],msg->local_name[1],msg->local_name[2],msg->local_name[3]);
    
    memcpy(thePairing->localname, (const uint8 *)msg->local_name, length);
    
    //DEBUG_LOGF("appHandleClDmLocalNameComplete, name:%x,%x,%x,%x", thePairing->localname[0],thePairing->localname[1],thePairing->localname[2],thePairing->localname[3]);
    //DEBUG_LOGF("appHandleClDmLocalNameComplete, name:%x,%x,%x,%x", thePairing->localname[4],thePairing->localname[5],thePairing->localname[6],thePairing->localname[7]);
    //DEBUG_LOGF("appHandleClDmLocalNameComplete, name:%x,%x,%x,%x", thePairing->localname[8],thePairing->localname[9],thePairing->localname[10],thePairing->localname[11]);
    //DEBUG_LOGF("appHandleClDmLocalNameComplete, name:%c,%c,%c,%c\r\n", thePairing->localname[0],thePairing->localname[1],thePairing->localname[2],thePairing->localname[3]);
    //DEBUG_LOGF("appHandleClDmLocalNameComplete, name:%c,%c,%c,%c\r\n", thePairing->localname[4],thePairing->localname[5],thePairing->localname[6],thePairing->localname[7]);
    //DEBUG_LOGF("appHandleClDmLocalNameComplete, name:%c,%c,%c,%c\r\n", thePairing->localname[8],thePairing->localname[9],thePairing->localname[10],thePairing->localname[11]);

    //appCheckDevNameValidation();
}
#endif
    ...
    switch (appPairingGetState(thePairing))
    {
        case PAIRING_STATE_IDLE:
        {
            ...
            else
            {
                ...
                /* check if we are attempting to page another handset */
                conManagerTaskData *theConMgr = appGetConManager();
                conManagerDevice *device;
                for (int i = 0; i < CON_MANAGER_MAX_DEVICES; i++)
                {
                    device = &theConMgr->devices[i];
                    /* the following is the condition to check if we are trying to page someone */
                    if (!BdaddrIsZero(&device->addr) && !appDeviceIsPeer(&device->addr) && device->state == ACL_CONNECTING)
                    {
                        DEBUG_LOGF("appHandleInternalHandsetPairRequest, device in ACL_CONNECTING. Releasing bdaddr %04x,%02x,%06lx.",device->addr.nap, device->addr.uap, device->addr.lap);
                        /* drop the ACL link (paging proc at this stage) */
                        while(device->users)
                        {
                            appConManagerReleaseAcl(&device->addr);
                        }
                        /* cleanup the AV states if any */
                        avInstanceTaskData *av_inst = appAvInstanceFindFromBdAddr(&device->addr);
                        if(av_inst)
                        {
                            DEBUG_LOGF("appHandleInternalHandsetPairRequest, found AV instance bdaddr %04x,%02x,%06lx. Cancelling.",device->addr.nap, device->addr.uap, device->addr.lap);

                            /* Cancel the messages to create A2DP and AVRCP connections */
                            MessageCancelAll(&av_inst->av_task, AV_INTERNAL_A2DP_CONNECT_REQ);
                            MessageCancelAll(&av_inst->av_task, AV_INTERNAL_AVRCP_CONNECT_REQ);

                            /* In theory the instance can be destroyed because we are not using it anymore */
                            appAvInstanceDestroy(av_inst);
                        }
                        /* Cancel the message to create HFP connection */
                        MessageCancelAll(appGetHfpTask(), HFP_INTERNAL_HFP_CONNECT_REQ);
...
void appPairingPeerPairingChange(void)
{
    pairingTaskData *thePairing = appGetPairing();
    
    MessageCancelAll(&thePairing->task, PAIRING_INTERNAL_PEER_PAIRING_CHANGE);
    MessageSendLater(&thePairing->task, PAIRING_INTERNAL_PEER_PAIRING_CHANGE,NULL,D_SEC(1));
}
...
static void appPairingHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
...
#ifdef APP_CHECK_DEV_NAME_VALIDATION
            appHandleGetLoalName(thePairing, (CL_DM_LOCAL_NAME_COMPLETE_T *)message);
#endif
        ...
        case PAIRING_INTERNAL_PEER_PAIRING_CHANGE:
            DEBUG_LOG("appPairingHandleMessage PAIRING_INTERNAL_PEER_PAIRING_CHANGE");
            if(appPeerSyncIsPeerPairing()&& appPeerSyncIsComplete()&&!appDeviceIsHandsetConnected()){
                appUiIdleActive();
            }
            else if(!appPeerSyncIsPeerPairing()&&appPeerSyncIsComplete()&&!appDeviceIsHandsetConnected() &&!appPeerSyncIsPeerHandsetAvrcpConnected()){
                appUiIdleActive();
            }
            else{
                appUiIdleInactive();
