...
static void appHfpEnterConnected(void)
{
    ...
    appPowerOffTimerDisable(APP_POWER_EVENT_HFP);
}
...
static void appHfpExitConnected(void)
{
    ...
    appPowerOffTimerEnable(APP_POWER_EVENT_HFP);
...
static void appHfpEnterConnectedActive(void)
{
    ...
    appAvStreamingSuspend(AV_SUSPEND_REASON_HFP);
#endif    
#if 0 //B380 disable calling led indication.
    ...
    switch (appHfpGetState())
    {
        ...
        case HFP_STATE_CONNECTED_ACTIVE:
        {
            ...
            */
            ...
            bool accept = (is_tws_plus && local_in_ear) || (!is_tws_plus && (local_in_ear || peer_in_ear));
...
static void appHfpHandleHfpHfIndicatorsInd(const HFP_HF_INDICATORS_IND_T *ind)
{
    ...
    if (ind->hf_indicator_assigned_num == hf_battery_level)
    {
...
#endif        
        {
            ...
            result = HfpXaplIndStatusInd(hfp_primary_link, hf_battery_level, appBatteryGetPercent());
            DEBUG_LOGF("appHfpHandleHfpHfIndicatorsInd HfpXaplIndStatusInd %d", result);
            MessageSendLater(appGetHfpTask(), HFP_INTERNAL_HFP_EACCEV_IND, 0, D_SEC(1));
        }
#if 0        
        else
        {
...
void appHfpFouceSendXapl(void){
    int16         result = 0;
    
    result = HfpXaplIndStatusInd(hfp_primary_link, hf_battery_level, appBatteryGetPercent());
    DEBUG_LOGF("appHfpFouceSendXapl HfpXaplIndStatusInd %d", result);
    MessageSendLater(appGetHfpTask(), HFP_INTERNAL_HFP_EACCEV_IND, 0, D_SEC(1));
}
...
static void appHfpHandleBatteryLevelUpdatePercent(MESSAGE_BATTERY_LEVEL_UPDATE_PERCENT_T *msg)
{
    ...
    switch (appHfpGetState())
    {
        ...
        case HFP_STATE_CONNECTED_ACTIVE:
            ...
            result = HfpAccevIndStatusInd(hfp_primary_link, hf_battery_level, msg->percent);
            DEBUG_LOGF("appHfpHandleBatteryLevelUpdatePercent HfpXaplIndStatusInd %d", result);
            break;
...
void appHfpCallVoice(void)
{
    ...
    switch (appHfpGetState())
    {
        ...
        {    
            /* Play tone */
            //appUiHfpVoiceDial();  //cjunfeng 20190325          
...
static void appHfpHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case HFP_INTERNAL_HFP_EACCEV_IND:
            result = HfpAccevIndStatusInd(hfp_primary_link, hf_battery_level, appBatteryGetPercent());
            DEBUG_LOGF("appHfpHandleMessage HfpXaplIndStatusInd %d", result);
            return;
    }
