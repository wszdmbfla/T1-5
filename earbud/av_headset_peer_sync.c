...
bool g_peer_master_in_case_flag = FALSE;
#endif
...
#if defined(MEDIA_AUTO_RESUME_PLAY)
        if (appDeviceIsHandsetAvrcpConnected() || appSmIsInCase())
        {
          g_peer_master_in_case_flag = FALSE;
        }
        else
        {
          g_peer_master_in_case_flag = TRUE;
        }
#endif
    }
...
static void appPeerSyncHandlePeerSigMsgChannelTxInd(const PEER_SIG_MSG_CHANNEL_RX_IND_T *ind)
{
    peerSyncTaskData* ps = appGetPeerSync();
    uint8 was_supported;
    bool was_peer_is_pairing = ps->peer_is_pairing;
    ...
    if(was_peer_is_pairing != ps->peer_is_pairing){
        appPairingPeerPairingChange();
    }
}
