...
static uint16 *appConManagerCreateAclImpl(const bdaddr *addr, bool is_ble)
{
    ...
    /* store the page timeout in the device struct */
    device->residual_page_timeout=page_timeout;

    /* if the page_timeout is more than 16 bits cut it to 0xFFFF */
    if (page_timeout > 0xFFFF)
        page_timeout = 0xFFFF;

    /* calculate the new residual timeout
     * if the ACL opened indication will find it !=0 it will page again setting the new timeout
    */
    device->residual_page_timeout-=page_timeout;
...
static void appConManagerHandleClDmAclOpenedIndication(const CL_DM_ACL_OPENED_IND_T *ind)
{
    ...
    else
    {
        if(ind->status == hci_error_page_timeout)
        {
            ...
            if(device && device->residual_page_timeout)
            {
                uint32 page_timeout;
...
static void appConManagerHandleClSmAuthoriseIndication(const CL_SM_AUTHORISE_IND_T *ind)
{
    ...
    if (!ble_connection)
    {
        ...
        else 
        {
            ...
            if (handset && theConMgr->handset_connect_allowed && !appPeerSyncIsPeerPairing())
            {
                DEBUG_LOG("appConManagerHandleClSmAuthoriseIndication, ALLOW handset (bredr)");
                authorise = TRUE;
            }
            else
            {
...
/*! \brief get device conenct time,the addr device must be handset. */
uint32 appConManagerDeviceResidualPageTimeGet(const bdaddr *addr)
{
    uint32 residualpagetime = 0;
    if(appDeviceIsHandset(addr)){
        conManagerDevice *device = appConManagerFindDeviceFromBdAddr(addr);
        if(device && device->residual_page_timeout)
            residualpagetime = device->residual_page_timeout * 625UL / 1000UL;
    }
    else
        DEBUG_LOG("Addr isn't Handset");

    DEBUG_LOGF("appConManagerDeviceResidualPageTimeGet time(ms):",residualpagetime);
    return residualpagetime;
}
