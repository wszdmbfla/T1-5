...
static void appPhyStateEnterInCase(void)
{
    ...
    if(appGetUi()->is_dut == TRUE){
        DEBUG_LOG("appPhyStateEnterInCase: in DUT so reboot!!!");
        appPowerReboot();
    }
}
...
static void appPhyStateHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case PHY_STATE_INTERNAL_IN_CASE_RECHECK:
            appPhyStateInCaseReCheckEvent();
            break;
...
void appPhyStateInCaseReCheckEvent(void)
{
    phyStateTaskData* phy_state = appGetPhyState();
	 DEBUG_LOG("appPhyStateInCaseReCheckEvent-------------1");

   if(phy_state->incasechecknum < 2)
   {
     if((appSmIsInCase() && appPeerSyncIsPeerInCase()))
     {
   	  DEBUG_LOG("appPhyStateInCaseReCheckEvent-------------2");
      
      if(appDeviceIsHandsetA2dpConnected() || appDeviceIsHandsetHfpConnected())
       {
         DEBUG_LOG("appPhyStateInCaseReCheckEvent-------------3");
         appConnRulesSetEvent(appGetSmTask(), RULE_EVENT_IN_CASE);
       }
     }
   }

   phy_state->incasechecknum++;

   if(phy_state->incasechecknum == 1)
   {
     DEBUG_LOG("appPhyStateInCaseReCheckEvent-------------4");
     MessageCancelAll(&phy_state->task, PHY_STATE_INTERNAL_IN_CASE_RECHECK);
     MessageSendLater(&phy_state->task, PHY_STATE_INTERNAL_IN_CASE_RECHECK,NULL, 3000);
   }
}
...
{
	  DEBUG_LOG("appPhyStateInCaseEvent-------------1");
    phyStateTaskData* phy_state = appGetPhyState();
...
{
	  DEBUG_LOG("appPhyStateOutOfCaseEvent-------------1");
    phyStateTaskData* phy_state = appGetPhyState();
