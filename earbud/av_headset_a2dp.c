                                   ...
                                   bool response_required)
{
    ...
    if (appA2dpIsSinkNonTwsCodec(theInst))
    {
        ...
        if (theOtherInst)
        {
            ...
            message->task = response_required ? &theInst->av_task : NULL;
            ...
            PanicFalse(MessageCancelAll(&theOtherInst->av_task, AV_INTERNAL_A2DP_INST_SYNC_IND) <= 1);
...
void appA2dpSignallingConnectIndicationNew(avTaskData *theAv, const A2DP_SIGNALLING_CONNECT_IND_T *ind)
{
    ...
    if (av_inst == NULL)
    {
        ...
        if (av_inst == NULL)
        {
            /* if we are here it means that there were no free entries in the instance table
             we are probably in the situation where we are trying to page another phone
             but we are paged by another one who wants to connect the A2DP signalling.
             Find the instance that is :
               - owned by a handset
               - the A2DP state is DISCONNECTED
             and reallocate it to this handset
            */
            DEBUG_LOG("appA2dpSignallingConnectIndicationNew, no free instances, attempting to replace an unused one");

            av_inst=appAvInstanceReplaceDisconnectedHandset(&ind->addr);
            if (av_inst)
            {
                DEBUG_LOG("appA2dpSignallingConnectIndicationNew, found an unused one. Replacing.");
...
#if defined(MEDIA_AUTO_RESUME_PLAY)
                MessageCancelAll(appGetUiTask(), AV_INTERNAL_MEDIA_PLAY_RESUME);
#endif
...
{
    a2dp_codec_settings *codec_settings = A2dpCodecGetSettings(ind->device_id, ind->stream_id);
    if (codec_settings)
    {
                                                        ...
                                                        const AV_INTERNAL_A2DP_INST_SYNC_IND_T *ind)
{
    ...
    MessageId msg_id = AV_INTERNAL_A2DP_TOP;
...
#define PER_REASON_DEBUG(REASON) DEBUG_LOGF("appA2dpHandleInternalA2dpInstSyncIndication(%p) state(0x%x) reason(" ## #REASON ## ")", theInst, local_state)
    ...
    switch (ind->reason)
    {
        case A2DP_INST_SYNC_REASON_MEDIA_CONNECTING:
        {
                ...
                (source_seid != AV_SEID_INVALID))
            {
                ...
                message->delay_ms = 0;
                msg_data = message;
                msg_id = AV_INTERNAL_A2DP_CONNECT_MEDIA_REQ;
            }
        }
        break;
        case A2DP_INST_SYNC_REASON_MEDIA_DISCONNECTED:
        {
                ...
                (theInst->a2dp.current_seid == source_seid))
            {
                ...
                msg_id = AV_INTERNAL_A2DP_DISCONNECT_MEDIA_REQ;
            }
        }
        break;
            ...
            switch (local_state)
            {
                ...
                case A2DP_STATE_CONNECTED_MEDIA_SUSPENDED:
                    if (theInst->a2dp.current_seid == source_seid)
                    {
                        ...
                        msg_data = message;
                        msg_id = AV_INTERNAL_A2DP_RESUME_MEDIA_REQ;
        ...
        case A2DP_INST_SYNC_REASON_CODEC_RECONFIGURED:
        {
            ...
            msg_data = message;
            msg_id = AV_INTERNAL_A2DP_CODEC_RECONFIG_IND;
            ...
            switch (local_state)
            {
#if defined(MEDIA_AUTO_RESUME_PLAY)
                case A2DP_STATE_DISCONNECTED:
                    DEBUG_LOG("A2DP_STATE_DISCONNECTED--------1");
                    if(g_peer_master_in_case_flag)
                    {
                        DEBUG_LOG("A2DP_STATE_DISCONNECTED--------2");
                        MessageCancelAll(appGetUiTask(), AV_INTERNAL_MEDIA_PLAY_RESUME);
                        MessageSendLater(appGetUiTask(), AV_INTERNAL_MEDIA_PLAY_RESUME, 0, 500);
                    } 
                    break;
                ...
                case A2DP_STATE_CONNECTED_MEDIA_STREAMING:
                {
                    if (theInst->a2dp.current_seid == source_seid)
                    {
                        ...
                        message->reason = AV_SUSPEND_REASON_RELAY;
                        msg_data = message;
                        msg_id = AV_INTERNAL_A2DP_SUSPEND_MEDIA_REQ;
                    }
                }
                break;
                case A2DP_STATE_CONNECTED_MEDIA_STREAMING_MUTED:
    ...
    if (AV_INTERNAL_A2DP_TOP != msg_id)
    {
    ...
    if (ind->task)
    {
        ...
        MessageSendConditionally(ind->task, AV_INTERNAL_A2DP_INST_SYNC_RES, message,
                                 (AV_INTERNAL_A2DP_TOP == msg_id)  ? NULL : &appA2dpGetLock(theInst));
    }
...
static void appA2dpHandleA2dpLinkLossInd(avInstanceTaskData *theInst, const A2DP_SIGNALLING_LINKLOSS_IND_T *ind){
    UNUSED(theInst);
    DEBUG_LOGF("appA2dpHandleA2dpLinkLossInd %d", ind->device_id);
}
...
void appA2dpInstanceHandleMessage(avInstanceTaskData *theInst, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case A2DP_SIGNALLING_LINKLOSS_IND:
            appA2dpHandleA2dpLinkLossInd(theInst, (A2DP_SIGNALLING_LINKLOSS_IND_T *)message);
            return;
