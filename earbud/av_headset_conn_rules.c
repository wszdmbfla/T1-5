...
static ruleAction ruleAutoHandsetPair(void)
{
    ...
    {
        if((appPeerSyncIsPeerInCase()&&!appPeerSyncHasPeerHandsetPairing())||
        	 (!appPeerSyncIsPeerInCase()&&appConfigIsLeft()&&!appPeerSyncHasPeerHandsetPairing()))
        {
    ...
    {
        if(appPeerSyncIsPeerInCase()&&!appDeviceGetHandsetBdAddr(NULL))
        {
            RULE_LOG("ruleAutoHandsetPair, run, no paired handset, we're out of case, peer is in case, we're right earbud");
            return RULE_ACTION_RUN;
        }
        else
        {
...
static bool ruleLowbatteryHandover(void)
{
    uint16 battery_level, peer_battery_level;

    appPeerSyncGetPeerBatteryLevel(&battery_level, &peer_battery_level);

    DEBUG_LOGF("ruleLowbatteryHandover local:%d,peer:%d",battery_level,peer_battery_level);

    if((battery_level >= 3400) &&(peer_battery_level <= 3350))
        return TRUE;
    else
        return FALSE;
}
...
static ruleAction ruleConnectHandsetStandard(ruleConnectReason reason)
{
    ...
    {   
    	  if((reason == RULE_CONNECT_USER) && (!appPeerSyncIsPeerInCase())) 
        { 
          if(ruleLowbatteryHandover()){
              DEBUG_LOG("ruleConnectHandsetStandard run handover");
              return RULE_ACTION_RUN;
          }
          else{
             RULE_LOG("ruleConnectHandsetStandard, peer earbud request handover handset, this time peer is outof case, ignore this hand over ############");
             return RULE_ACTION_IGNORE;
          }
        }

        appSmLinklossConnectHandset();
        ...
        if (appSmIsOutOfCase())
        {
            ...
            else
            {
                if (appPeerSyncPeerRulesInProgress())
                {
                    ...
                    if(appConfigIsLeft()&&(appConnRulesInProgress()
                    	||!appPeerSyncHasPeerHandsetPairing()))
                    {
                      RULE_LOG("ruleConnectHandsetStandard, ignore as left is out of case but has rules in progress");
                    }
                    else
                    {
                    	RULE_LOG("ruleConnectHandsetStandard, ignore as right is out of case but has rules in progress");
                      return RULE_ACTION_IGNORE;
                    }
                }
...
static ruleAction ruleInEarAncEnable(void)
{
    RULE_LOG("ruleInEarAncEnable, run");
    return RULE_ACTION_RUN;
}

static ruleAction ruleOutOfEarAncDisable(void)
{
    RULE_LOG("ruleOutOfEarAncDisable, run");
    return RULE_ACTION_RUN;
}
...
{
    return appDeviceIsHandsetConnected() && !appSmIsDfuPending() && !appScoFwdIsSending();
}
...
static ruleAction ruleBleConnectionUpdate(void)
{
    ...
    if (connectable && peer)
    {
        ...
        if (peer_handset || peer_dfu || (peer_ble_advertising && ! peer_in_case) || peer_ble_connected)
        {
            ...
            if((!peer_handset && !appPeerSyncIsPeerPairing())||peer_ble_connected){
                connectable = TRUE;
            }
            else
            connectable = FALSE;
        ...
        else if (!peer_in_case)
        {
            ...
            if(appDeviceIsHandsetConnected()){
                DEBUG_LOG("ruleBleConnectionUpdate local connect handset& BLE enable");
                connectable = TRUE;
            }
            else{
