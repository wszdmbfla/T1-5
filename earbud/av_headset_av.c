...
avInstanceTaskData *appAvInstanceReplaceDisconnectedHandset(const bdaddr *bd_addr)
{
    avTaskData *theAv = appGetAv();
    avInstanceTaskData *theInst;
    int instance;
    ...
    for (instance = 0; instance < AV_MAX_NUM_INSTANCES; instance++)
    {
        ...
        if (!appDeviceIsPeer(&theInst->bd_addr) && theInst->a2dp.state == A2DP_STATE_DISCONNECTED)
        { 
            ...
            DEBUG_LOGF("appAvInstanceReplaceDisconnectedHandset %p", theInst);
