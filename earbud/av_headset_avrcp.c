...
static bool avrcp_play_status_pause_waiting = FALSE;
static void appAvrcpHandleInternalAvrcpPlayToggleRequest(avInstanceTaskData *theInst)
{
    ...
    {
        if ((appAvPlayStatus() == avrcp_play_status_playing)&&(avrcp_play_status_pause_waiting == FALSE))
        {
            appAvrcpRemoteControl(theInst, opid_pause, 0, FALSE, 0);
            appAvrcpRemoteControl(theInst, opid_pause, 1, FALSE, 0);
            appAvHintPlayStatus(avrcp_play_status_paused);
            ...
            MessageCancelAll(&theInst->av_task, AV_INTERNAL_AVRCP_PLAY_TOGGLE_REQ_TIMEOUT);
            MessageSendLater(&theInst->av_task, AV_INTERNAL_AVRCP_PLAY_TOGGLE_REQ_TIMEOUT, NULL, D_SEC(2));
        }
        else if ((appAvPlayStatus() != avrcp_play_status_playing) || (avrcp_play_status_pause_waiting==TRUE))
        {
            appAvrcpRemoteControl(theInst, opid_play, 0, FALSE, 0);
            appAvrcpRemoteControl(theInst, opid_play, 1, FALSE, 0);
            appAvHintPlayStatus(avrcp_play_status_playing);
            appAvrcpSetPlaybackLock(theInst);
            avrcp_play_status_pause_waiting = FALSE;
        }
    }
}

static void appAvrcpHandleInternalAvrcpPlayToggleTimeout(avInstanceTaskData *theInst)
{
	  UNUSED(theInst);
    DEBUG_LOG("appAvrcpHandleInternalAvrcpPlayToggleTimeout");
    avrcp_play_status_pause_waiting = TRUE;
}
            ...
            switch (theInst->avrcp.op_id)
            {
                ...
                default:
                {
                    ...
                    if ((theInst->avrcp.op_state == 0) && (cfm->status != avrcp_success)){
                        ...
                        DEBUG_LOG("passthrough failed, reconnect avrcp");
                        
                        MessageSend(appGetSmTask(), CONN_RULES_DISCONNECT_PEER, 0);
                        MessageSendLater(appGetSmTask(), CONN_RULES_CONNECT_PEER, 0, D_SEC(1));
                    }
                }
                break;
            }               
...
void appAvrcpInstanceHandleMessage(avInstanceTaskData *theInst, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case AV_INTERNAL_AVRCP_PLAY_TOGGLE_REQ_TIMEOUT:
            appAvrcpHandleInternalAvrcpPlayToggleTimeout(theInst);
            return;        	  
    ...
    switch (id)
    {
        ...
        case AVRCP_SET_ABSOLUTE_VOLUME_IND:
            appAvrcpHandleSetAbsoluteVolumeInd(theInst, (AVRCP_SET_ABSOLUTE_VOLUME_IND_T *) message);
            return;
