...
extern void appUiPeerSyncPoweroff(void);
#endif
...
static void appScoFwdHandleOTARingCancel(void)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();

    if(MessageCancelAll(&theScoFwd->task, SFWD_INTERNAL_PLAY_RING))
    {
        DEBUG_LOG("appScoFwdHandleOTARingCancel, Pending message to RING cancelled");
    }
...
static void ProcessOTAControlMessage(uint8 ota_msg_id, const uint8* payload, int payload_size)
{
    ...
    switch (ota_msg_id)
    {
        ...
        case SFWD_OTA_MSG_CALL_VOICE_SIRI_ENABLE:
            DEBUG_LOG("SFWD_OTA_MSG_CALL_VOICE_SIRI_ENABLE");
            VoiceRecognitionEnable();
            break;

        case SFWD_OTA_MSG_CALL_VOICE_SIRI_DISABLE:
            DEBUG_LOG("SFWD_OTA_MSG_CALL_VOICE_SIRI_DISABLE");
            VoiceRecognitionDisable();
            break;
                       
        case SFWD_OTA_MSG_AV_PLAY_TOGGLE:
        	  DEBUG_LOG("SFWD_OTA_MSG_AV_PLAY_TOGGLE");
        ...
        case SFWD_OTA_MSG_HEART_BEAT:
            DEBUG_LOG("received head beat");
            break;
        ...
        case SFWD_OTA_MSG_RING_CANCEL:
            DEBUG_LOG("SCO Forwarding PEER ring cancel");
            appScoFwdHandleOTARingCancel();
        break;
        ...
        case SFWD_OTA_MSG_MICFWD_STOP:
            ...
            break;
       #if defined(TWS_PEER_SYNC_POWEROFF)
        case SFWD_OTA_MSG_PEER_POWEROFF:
         {
            DEBUG_LOG("SFWD_OTA_MSG_PEER_POWEROFF");
            appUiPeerSyncPoweroff();
        }
        break;
       #endif
      #if defined(PEER_FORCE_PAIRING)
        case SFWD_OTA_MSG_PEER_FORCED_PAIRING:
        {
            DEBUG_LOG("SFWD_OTA_MSG_PEER_FORCED_PAIRING");
            uiPeerReadytoForcedPairing();
            }
            break;
     #endif

    #if defined(INCLUDE_EQ)
    case SFWD_OTA_MSG_UPDATE_USERCONFIG:
    {
        DEBUG_LOG("SFWD_OTA_MSG_UPDATE_USERCONFIG");
        appScoFwdHandlePeerSyncUserConfig(payload, payload_size);
    }
    break;
    #endif
        default:
...
static void appScoFwdClearForwarding(void)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();
...
static void appScoFwdClearForwardingReceive(void)
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();
...
static void appScoFwdHandleEnableForwarding(void)
{
    ...
    {
        if (appLinkPolicyGetCurrentScoFwdRole() != hci_role_master)
        {
...
void appSocHfpFouceSIRIEnable(void)
{
   DEBUG_LOG("appSocHfpFouceSendAPLSIRI");
   SendOTAControlMessage(SFWD_OTA_MSG_CALL_VOICE_SIRI_ENABLE);
}

void appSocHfpFouceSIRIDisable(void)
{
   DEBUG_LOG("appSocHfpFouceSendAPLSIRI");
   SendOTAControlMessage(SFWD_OTA_MSG_CALL_VOICE_SIRI_DISABLE);
}

void appSocHfpFouceSendAvPlayToggle(void)
{
   DEBUG_LOG("appSocHfpFouceSendAvPlayToggle");
   //appUiButton();
   SendOTAControlMessage(SFWD_OTA_MSG_AV_PLAY_TOGGLE);
}
...
#if defined(TWS_PEER_SYNC_POWEROFF)
void appSocHfpPeerSyncPoweroff(void)
{
   DEBUG_LOG("appSocHfpPeerSyncPoweroff");
   SendOTAControlMessage(SFWD_OTA_MSG_PEER_POWEROFF);
}
#endif

#if defined(PEER_FORCE_PAIRING)
void appSOCHfpPeerForcedEntryPairing(void)
{
    DEBUG_LOG("appSOCHfpPeerForcedEntryPairing");
    SendOTAControlMessage(SFWD_OTA_MSG_PEER_FORCED_PAIRING);
}
#endif
...
void appScoFwdHeartBeat(void)
{
    DEBUG_LOG("appScoFwdHeartBeat");
    SendOTAControlMessage(SFWD_OTA_MSG_HEART_BEAT);
}
...
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();
    SendOTAControlMessage(SFWD_OTA_MSG_RING_CANCEL);
    if(MessageCancelAll(&theScoFwd->task, SFWD_INTERNAL_PLAY_RING))
    {
...
{
    scoFwdTaskData *theScoFwd = appGetScoFwd();
    uint16 volume = appHfpGetVolume();
    uint8 delay = 0;
    bool allow_scofwd = FALSE;

    DEBUG_LOGF("appScoFwdHandleHfpAudioConnectConfirmation %u", cfm->status);

    if (appConfigScoForwardingEnabled())
    {
        /* Assume connected to a TWS+ handset, which will disable SCOFWD */
        bool tws_plus_handset = TRUE;
        bdaddr hfp_bdaddr;

        /* Query HFP for the Bluetooth Device Address  */
        if (HfpLinkGetBdaddr(cfm->priority, &hfp_bdaddr))
        {
            tws_plus_handset = appDeviceIsTwsPlusHandset(&hfp_bdaddr);
        }
        allow_scofwd = !tws_plus_handset;
        ...
        /* SCOFWD has decided local volume should be muted, so start SCO as such */
        if (theScoFwd->vol_muted)
        {
            volume = 0;
        }
    }
    ...
    appKymeraScoStart(cfm->audio_sink, cfm->codec, cfm->wesco, volume, delay, allow_scofwd);
...
{
    UNUSED(ind);
    DEBUG_LOGF("appScoFwdHandleHfpAudioDisconnectIndication %u", ind->status);
    ...
    if (appConfigScoForwardingEnabled())
    {
        switch (appScoFwdGetState())
        {
            ...
            case SFWD_STATE_IDLE:
                /* Nothing to do, SFWD is IDLE, the L2CAP went down before
                 * notification SCO going down arrived */
                break;

            case SFWD_STATE_CONNECTED_ACTIVE_RECEIVE:
                DEBUG_LOG("appScoFwdHandleHfpAudioDisconnectIndication bad SFWD state SFWD_STATE_CONNECTED_ACTIVE_RECEIVE");
                Panic();
                break;

            default:
                DEBUG_LOGF("appScoFwdHandleHfpAudioDisconnectIndication unexpected SFWD state %u", appScoFwdGetState());
                break;
        }
    }
...
{
    DEBUG_LOG("appScoFwdSetWallclock");
    scoFwdTaskData *theScoFwd = appGetScoFwd();
    theScoFwd->wallclock_sink = sink;
}

#if defined(INCLUDE_EQ)
void appScoFwdPeerSyncUserConfig(uint16 mode)
{
    uint8 t_data = mode;
    DEBUG_LOGF("appScoFwdPeerSyncUserConfig %d",mode);
    SendOTAControlMessageWithPayload(SFWD_OTA_MSG_UPDATE_USERCONFIG,&t_data,sizeof(t_data));
}

void appScoFwdHandlePeerSyncUserConfig(const uint8* msg, int msg_size)
{
    uint16 t_mode = msg[0];
    DEBUG_LOGF("appScoFwdHandlePeerSyncUserConfig size:%d,msg[0]:%d", msg_size,msg[0]);
    appUiUpdateEQ(t_mode,TRUE);
}
