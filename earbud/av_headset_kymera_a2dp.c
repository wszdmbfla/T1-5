...
static bool appKymeraA2dpStartMaster(const a2dp_codec_settings *codec_settings, uint8 volume)
{
    ...
    switch (appKymeraGetState())
    {
        ...
        case KYMERA_STATE_A2DP_STARTING_C:
        {
            ...
            appKymeraSetOperatorUcids(FALSE, FALSE);
...
void appKymeraHandleInternalA2dpUpdateEq(void)
{
    ...
    DEBUG_LOG("appKymeraHandleInternalA2dpUpdateEq");

    switch (theKymera->state){
    case KYMERA_STATE_A2DP_STREAMING:
    case KYMERA_STATE_A2DP_STREAMING_WITH_FORWARDING:
        peq_op = ChainGetOperatorByRole(theKymera->chainu.output_vol_handle, OPR_PEQ);
        if(peq_op){
            DEBUG_LOGF("appKymeraHandleInternalA2dpUpdateEq %d", appui_get_eq_mode());
