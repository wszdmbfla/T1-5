...
static void appEnterSubStateTerminating(void)
{
    ...
    appSmInitiateLinkDisconnection(SM_DISCONNECT_ALL, 1000,
                                   POST_DISCONNECT_ACTION_NONE);                                
    }
    else{
...
void appSetState(appState new_state)
{
     ...
     */
    if ((new_state != previous_state) && !appSmStateIsSleepy(new_state)){
    ...
    switch (appGetState())
    {
        case APP_STATE_PEER_PAIRING:
        {
            ...
            if (cfm->status == pairingSuccess){
                ...
                /* if peer pairing not successful don't try again
                 * modify by Takoo at 20180627  */                
                appSetState(APP_STATE_IN_EAR_IDLE);
                appGetSm()->phy_state = PHY_STATE_IN_EAR;
            }
        }
        break;
...
static void appSmHandleConnRulesHandsetPair(void)
{
    ...
    switch (appGetState())
    {
        ...
        case APP_STATE_IN_EAR_IDLE:
            ...
            break;
        default:
            DEBUG_LOG("appSmHandleConnRulesHandsetPair %d don't allow pair with handset", appGetState());
            break;
    }
}
...
static void appSmHandleConnRulesConnectHandset(CONN_RULES_CONNECT_HANDSET_T* crch)
{
    ...
    if (crch->profiles & DEVICE_PROFILE_HFP)
    {
        ...
        if(appDeviceIsTwsPlusHandset(appHfpGetAgBdAddr()))
        {
          if(appConfigIsLeft())
          {
            MessageCancelAll(appGetSmTask(), SM_INTERNAL_RECONNECT_HANDSET_FAILED);
            MessageSendLater(appGetSmTask(), SM_INTERNAL_RECONNECT_HANDSET_FAILED, 0, 10500);
            DEBUG_LOG("appSmHandleConnRulesConnectHandset after 10s enter pairing---tws+");
          }
        }
        else
        {
          uint32 time = 0;
          bdaddr handsetaddr ={0};

          if(appDeviceGetHandsetBdAddr(&handsetaddr)){
              time = appConManagerDeviceResidualPageTimeGet(&handsetaddr);
          }

          if(MessageCancelAll(appGetSmTask(), SM_INTERNAL_CONNECT_LINKLOSS_HANDSET)&& time > 30000){
            DEBUG_LOG("handset connect because of handset linkloss,so don't send SM_INTERNAL_RECONNECT_HANDSET_FAILED ");
          }
          else{
            MessageCancelAll(appGetSmTask(), SM_INTERNAL_RECONNECT_HANDSET_FAILED);
            MessageSendLater(appGetSmTask(), SM_INTERNAL_RECONNECT_HANDSET_FAILED, 0, 10500); // a default pagetimeout
            DEBUG_LOG("appSmHandleConnRulesConnectHandset after 10s enter pairing");
          }
        ...
        default:
        {
            if (appDeviceIsHandset(&ind->bd_addr))
            {
                ...
                if (ind->reason == APP_HFP_DISCONNECT_NORMAL && !appSmIsDisconnectingLinks()){
                    ...
                    /* handset goto paring state */
                    if(NULL == appGetGaiaTransport()){
                        MessageCancelAll(appGetSmTask(), SM_INTERNAL_PAIR_HANDSET);
                        MessageSendLater(appGetSmTask(), SM_INTERNAL_PAIR_HANDSET, 0, 500);
                    }
                }

                if(appDeviceIsTwsPlusHandset(&ind->bd_addr))
                {
                  if(ind->reason == APP_HFP_CONNECT_FAILED && (appGetState() != APP_STATE_HANDSET_PAIRING) && !appPeerSyncIsPeerPairing()){
                    if(appConfigIsLeft())
                    {
                      MessageCancelAll(appGetSmTask(), SM_INTERNAL_PAIR_HANDSET);
                      MessageSendLater(appGetSmTask(), SM_INTERNAL_PAIR_HANDSET, 0, 500);
...
#if 0
static bool appSmBatteryVoltage(void)
{
    uint16 battery_level, peer_battery_level;

    appPeerSyncGetPeerBatteryLevel(&battery_level, &peer_battery_level);
    DEBUG_LOGF("appSmBatteryVoltage, battery %u, peer battery %u", battery_level, peer_battery_level);

    if (battery_level > peer_battery_level){
        DEBUG_LOG("appSmBatteryVoltage, run as our battery is higher");
        return TRUE;
    }
    else if (battery_level == peer_battery_level){
        if (appConfigIsLeft()){
            DEBUG_LOG("appSmBatteryVoltage, equal, run as left earbud");
            return TRUE;
        }
        else{
            DEBUG_LOG("appSmBatteryVoltage, equal, ignore as right earbud");
            return FALSE;
        }
    }
    else{
        DEBUG_LOG("appSmBatteryVoltage, ignore as our battery is lower");
        return FALSE;
    }
}
#endif
/*! \brief in out of case status reconnect handset
           if failed and peer sync complete then enter handset pairing           
    \note There must be peer sync complete */
static void appSmHandleInternalReconnectHandsetFailed(void)
{
    ...
    if (appPeerSyncIsComplete()){
        ...
        if (appPeerSyncIsPeerPairing()){
            DEBUG_LOG("appSmHandleInternalReconnectHandsetFailed, ignore as peer is pairing");
            return;
        }

        if (/*appSmBatteryVoltage() && */appSmStateIsIdle(appGetState())){
            DEBUG_LOG("appSmHandleInternalReconnectHandsetFailed, enter handset pairing");

            appSmSetUserPairing();
            appSetState(APP_STATE_HANDSET_PAIRING);
        }
        else{
            DEBUG_LOG("appSmHandleInternalReconnectHandsetFailed can only pair in IDLE state");
        }
    }
    else{
        DEBUG_LOG("appSmHandleInternalReconnectHandsetFailed, ignore as sync not complete");
    }
}
...
{
    /* it wait too long if at peer pairing or handset pairing state
       so fouce enter facotry reset
     */
    DEBUG_LOGF("appSmHandleInternalFactoryReset state(0x%08X)", appGetState());
...
void appSmHandleMessage(Task task, MessageId id, Message message)
{
    ...
    switch (id)
    {
        ...
        case SM_INTERNAL_PAIR_HANDSET:
            ...
            break;            
        case SM_INTERNAL_RECONNECT_HANDSET_FAILED:            
            appSmHandleInternalReconnectHandsetFailed();
            break;
...
void appSmLinklossConnectHandset(void)
{
    MessageCancelAll(appGetSmTask(), SM_INTERNAL_CONNECT_LINKLOSS_HANDSET);
    MessageSendLater(appGetSmTask(), SM_INTERNAL_CONNECT_LINKLOSS_HANDSET,NULL,D_SEC(10));
}

void appSmSingleEarbudPoweronConnectHandset(void)
{
    CONN_RULES_CONNECT_HANDSET_T* action; //= {0, RULE_POST_HANDSET_CONNECT_ACTION_NONE};
    bdaddr handset_addr;

    action =(CONN_RULES_CONNECT_HANDSET_T*) PanicUnlessMalloc(sizeof(CONN_RULES_CONNECT_HANDSET_T));
    memset(action, 0, sizeof(CONN_RULES_CONNECT_HANDSET_T));
    
    if(appDeviceGetHandsetBdAddr(&handset_addr)){
        action->profiles = appDeviceWasConnectedProfiles(&handset_addr);
        MessageSend(appGetSmTask(), CONN_RULES_CONNECT_HANDSET, (CONN_RULES_CONNECT_HANDSET_T*)action);
    }
